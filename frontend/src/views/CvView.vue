...
<template>
	<div class="min-h-screen bg-slate-100 print:bg-white text-slate-800 font-sans py-10 px-4 print:p-0">
		...
		<!-- Contenedor del CV -->
		<main
			class="max-w-7xl mx-auto bg-white p-8 md:p-12 shadow-xl print:shadow-none print:max-w-full print:p-0 rounded-xl print:rounded-none border border-slate-200 print:border-none">

			<!-- Header -->
			<header
				class="border-b-2 border-slate-800 pb-6 mb-6 flex flex-col sm:flex-row justify-between items-start gap-4">
				<div>
					<h1 class="text-6xl font-extrabold uppercase tracking-tight text-slate-900">
						{{ cvInfo.name }}
					</h1>
					<h2 class="text-3xl font-semibold text-emerald-700 print:text-slate-700 mt-1">
						{{ cvInfo.title }}
					</h2>
				</div>
				<div class=" text-slate-600 space-y-1 sm:text-right">
			...
			<!-- Sobre mí -->
			<section class="mb-6">
				<h3 class=" font-bold uppercase tracking-wider text-slate-900 border-b border-slate-300 pb-1 mb-2">
					Sobre Mí
				</h3>
			...
			<!-- Experiencia Laboral -->
			<section class="mb-6">
				<h3 class=" font-bold uppercase tracking-wider text-slate-900 border-b border-slate-300 pb-1 mb-3">
					Experiencia Laboral
				</h3>
				<div class="space-y-4">
					<div v-for="(exp, index) in cvInfo.experiences" :key="index">
						<div class="flex justify-between items-baseline">
							...
							<span class=" font-semibold text-slate-500">{{ exp.period }}</span>
						</div>
						<p class=" font-semibold text-emerald-700 print:text-slate-600 mb-1">{{ exp.company }}
						</p>
				...
				<section>
					...
					<div class="space-y-3">
						<div v-for="(edu, index) in cvInfo.education" :key="index">
							<span class=" font-bold text-slate-500 block">{{ edu.year }}</span>
				...
				<section>
					...
					<div class="flex flex-wrap gap-1.5">
						<span v-for="tech in cvInfo.techSkills" :key="tech"
							class="px-2 py-0.5 bg-slate-100 print:bg-slate-50 border border-slate-300 text-slate-800  font-medium rounded">
							{{ tech }}
						</span>
					</div>
				</section>
